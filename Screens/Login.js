...
export default function Login({navigation}) {
    ...
    const [request, response, promptAsync] = Google.useAuthRequest({
        ...
        iosClientId:"340004188318-rovsit0rj1k424kn7bdd3nu5jbsog2m3.apps.googleusercontent.com"
    })
    ...
    <TouchableWithoutFeedback style={{width: width, height:height, backgroundColor:'#030B27'}}> 
        ...
        <>
            <View style={tw` mb-1 items-center flex-row`}>
                ...
                <Text style={tw` ml-3 text-white text-4xl text-center`}>Login To Ignition</Text>
                </MotiView>
                    ...
                    try {
                        ...
                        requestedScopes: [
                            ...
                            if(!userToken.email){
                                ...
                                signInWithEmailAndPassword(auth, decoded.email, userToken.user).catch((error)=>{
                                    console.log(error)
                                    if(error.code == "auth/user-not-found"){
                                        createUserWithEmailAndPassword(auth, decoded.email, userToken.user)
                                    }
                                })
                            } else {
